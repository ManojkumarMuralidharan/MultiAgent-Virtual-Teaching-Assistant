...
public class ExamApplication extends Application implements ConnectionListener {
	...
	public void onCreate() {
		...
		if (mConnection == null) {
			...
			mConnection = new ServiceConnection() {
				public void onServiceConnected(ComponentName className,
					...
					// it.
					examService = ((ExamService.LocalBinder) service)
							.getService();
		...
		messageListener = new MessageListener(vTAgent);
		...
		Log.i(TAG, "JADE Registration Properties: " + msisdn + "@" + host + ":"
				+ port + "/JADE");
		Log.i(TAG, "JADE Other Agents: " + all_msisdn);

		try {
			Log.i(TAG, "Trying to connect to JADE Container ");
			JadeGateway.connect(VTAGatewayAgent.class.getName(), null, props,
					this, this);
		} catch (Exception e) {
	...
	public void onTerminate() {
		...
		Log.i(TAG,
				" onTerminate method is called. Agent will disconnect from JADE container.");
